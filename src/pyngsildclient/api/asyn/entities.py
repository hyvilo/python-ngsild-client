...
from ..exceptions import NgsiAlreadyExistsError, rfc7807_error_handle_async
...
class Entities:
    ...
    def to_broker_url(self, eid: str | Entity) -> str:
    ...
    @rfc7807_error_handle_async
    async def delete(self, eid: str | Entity) -> bool:
        eid = eid.id if isinstance(eid, Entity) else Urn.prefix(eid)
        logger.info(f"{eid=}")
    ...
    @rfc7807_error_handle_async
    async def exists(self, eid: str | Entity) -> bool:
        eid = eid.id if isinstance(eid, Entity) else Urn.prefix(eid)
