...
def _compute_node_trace(node_x, node_y, node_text, node_color, node_size) -> go.Scatter:
    node_trace = go.Scatter(
        x=node_x, y=node_y, mode="markers", hoverinfo="text", marker=dict(reversescale=True, line_width=2)
    )
...
def _mk_graph_fig(edge_trace, node_trace) -> go.Figure:
    fig = go.Figure(
        ...
        layout=go.Layout(
            ...
            annotations=[
                dict(text="Entities network graph", showarrow=False, xref="paper", yref="paper", x=0.005, y=-0.002)
            ],
