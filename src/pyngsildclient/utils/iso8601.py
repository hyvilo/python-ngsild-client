...
def from_datetime(value: datetime) -> str:
    ...
    >>> from pyngsildclient import iso8601
...
def from_string(type: Literal["DateTime", "Date", "Time"], value: str) -> datetime | date | time:
    with suppress(ValueError):
        if type == "DateTime":
...
def to_string(dt: datetime | date | time) -> tuple[str, str]:
...
def extract(value: str) -> datetime | None:
